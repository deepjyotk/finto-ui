...
export default function HitlScreenerPanel({ embedInPreview = false }: HitlScreenerPanelProps) {
  ...
  const handleAction: A2UIActionHandler = (action, clientDataModel) => {
    ...
    const validationErrors = Object.fromEntries(
      Object.entries(formValues)
        ...
        .map(([fieldName]) => [fieldName, ["Field cannot be empty"]])
    )

    if (Object.keys(validationErrors).length && hitlPayload?.surfaceId) {
      setLiveA2UIValidationErrors(hitlPayload.surfaceId, validationErrors)
      return
    }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div
        className="min-h-0 flex-1 overflow-y-auto px-4 pb-[max(6rem,calc(env(safe-area-inset-bottom)+5rem))] pt-4"
        style={{ scrollPaddingBottom: "max(6rem, calc(env(safe-area-inset-bottom) + 5rem))" }}
      >
        {taskHint && (
